 ...
 *
 * Known limitation: loops and conditionals don't currently have a
 * dedicated "continue after this" port (their outputs are fully used by
 * their internal branches/body), so nothing can be wired to run *after*
 * an if/else or loop finishes — only what's inside them. Adding that
 * would mean adding a distinct "next" port to those shapes.
 */
function generatePython(projectName: string | undefined, blocks: PlacedBlock[], connections: Connection[]) {
  ...
  function genChain(blockId: string, indent: string): string[] {
    if (visited.has(blockId)) return [];
    visited.add(blockId);
    const block = byId.get(blockId);
    if (!block) return [];
    const def = getBlockDef(block.defId);
    if (!def) return [`${indent}# unknown block`];
    const v = block.values;

    if (block.defId === "if.then") {
      const lines = [`${indent}if ${v.cond || "True"}:`];
      const trueConn = outByPort.get(`${blockId}:true`);
      const trueLines = trueConn ? genChain(trueConn.to, indent + "    ") : [];
      lines.push(...(trueLines.length ? trueLines : [`${indent}    pass`]));

      const falseConn = outByPort.get(`${blockId}:false`);
      if (falseConn) {
        lines.push(`${indent}else:`);
        const falseLines = genChain(falseConn.to, indent + "    ");
        lines.push(...(falseLines.length ? falseLines : [`${indent}    pass`]));
      }
      return lines;
    }
    ...
    if (block.defId === "loop.for" || block.defId === "loop.while") {
      ...
      const bodyLines = bodyConn ? genChain(bodyConn.to, indent + "    ") : [];
      lines.push(...(bodyLines.length ? bodyLines : [`${indent}    pass`]));
      return lines;
    }
    ...
    if (next && next.toPort !== "loopback") {
      lines.push(...genChain(next.to, indent));
    }
    return lines;
  }
  ...
  for (const r of roots) {
    bodyLines.push(...genChain(r.instanceId, "    "));
  }
  ...
  for (const b of blocks) {
    if (!visited.has(b.instanceId)) {
      bodyLines.push(...genChain(b.instanceId, "    "));
    }
  }
